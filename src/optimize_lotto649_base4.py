...
K1 = 49 
BACKTEST_DRAWS = 1000 
BASE_TICKETS = 4      # User requested minimum of 4 tickets
...
if __name__ == "__main__":
    ...
    caps = [14, 15, 16, 17, 18, 19, 20]
    out_data = []
    
    print(f"Starting Base-4 Optimization (N={N_SIMS})...")
    print(f"{'Cap':<5} | {'ROI':<8} | {'Avg Profit':<11} | {'Hits':<5} | {'Hit Locations (Draw Index)'}")
    print("-" * 100)
    
    for cap_max in caps:
        # 1. Deterministic Cost (Base 4..cap)
        curr_tickets = BASE_TICKETS
        total_cost = 0
        ...
        for sim in range(N_SIMS):
            rng = np.random.default_rng(sim + 20000) # Offset seed
            curr_tickets = BASE_TICKETS
            sim_prize = 0
            ...
            for i in range(start_index, total_draws):
                ...
                if jp_per_winner[i] > 0: curr_tickets = BASE_TICKETS
