...
TICKET_PRICE = 100
MAX_TICKETS = 15      # 上限調整為 15 注
N_SIMS = 1000        # 10k sims
...
def run_simulation():
    print(f"Initializing Hybrid Strategy (Always 1, Threshold 800m -> Cap-15)...")
    ...
    for sim in range(N_SIMS):
        ...
        current_n_tickets = 1 # 平常買 1 注
        in_high_stakes_mode = False 
        ...
        for i in range(500, len(df_chrono)):
            ...
            if current_jp >= THRESHOLD:
                # Enter or continue high stakes mode
                # If we were just maintaining 1 ticket, jump to 2 to start the ramp up?
                # Or just continue the dynamic logic from wherever we are.
                # Let's align with user request: "Start dynamic strategy".
                # Standard dynamic usually starts at 2 tickets.
                if not in_high_stakes_mode:
                    current_n_tickets = max(2, current_n_tickets) # Bump to at least 2
                    in_high_stakes_mode = True
                
                # Dynamic Logic for amount (handled at end of loop)
            else:
                # Low stakes mode
                current_n_tickets = 1
                in_high_stakes_mode = False
            ...
            # Update dynamic ticket count for NEXT draw
            # Rule: If Jackpot won -> Reset
            # Rule: If Jackpot NOT won -> Increase (up to Cap)
            if jp_per_winner[i] > 0:
                current_n_tickets = 1 # Reset to base
                in_high_stakes_mode = False # Reset mode
            else:
                if in_high_stakes_mode:
                    current_n_tickets = min(current_n_tickets + 1, MAX_TICKETS)
                # Else stays at 1 (handled at start of next loop)
...
if __name__ == "__main__":
    ...
    res_df.to_csv("../output/replay_result_hybrid_1_cap15_10k.csv", index=False)
