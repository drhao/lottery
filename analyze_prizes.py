...
def calculate_average_prizes(file_path):
    try:
        ...
        # Filter for First Prize winners (Per Bet > 0)
        first_prize_winners = df[df['First_Prize_Per_Bet'] > 0]['First_Prize_Per_Bet']
        avg_first_prize = first_prize_winners.mean()
        count_first_prize = first_prize_winners.count()

        # Filter for Second Prize winners (Per Bet > 0)
        second_prize_winners = df[df['Second_Prize_Per_Bet'] > 0]['Second_Prize_Per_Bet']
        ...
        if count_first_prize > 0:
            ...
            print(f"  Average Prize Per Bet: {avg_first_prize:,.2f}")
        else:
        ...
        if count_second_prize > 0:
            ...
            print(f"  Average Prize Per Bet: {avg_second_prize:,.2f}")
        else:
