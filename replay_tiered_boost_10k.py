...
THRESHOLD_START = 800000000 # 8-亿開始
THRESHOLD_BOOST = 1000000000 # 10-亿超頻
ALPHA = 100
PENALTY_BASE = 0.4
PENALTY_LARGE = 0.2
K1, K2 = 38, 8
...
def run_simulation():
    print(f"Initializing Tiered Strategy (800m->Cap10, 1000m->Cap15) N_SIMS={N_SIMS}...")
    ...
    for sim in range(N_SIMS):
        ...
        for i in range(500, len(df_chrono)):
            ...
            # --- THRESHOLD CHECK ---
            if current_jp < THRESHOLD_START:
                # Update counts, reset base momentum
                counts1[first_draws[i]] += 1
                counts2[second_draws[i]] += 1
                current_n_tickets_base = 2   
                continue
            
            # --- DETERMINE ACTUAL TICKETS ---
            # If jackpot >= 10亿, boost max cap to 15
            # If 8亿 <= jackpot < 10亿, max cap is 10
            
            cap = 10
            if current_jp >= THRESHOLD_BOOST:
                cap = 15
                
            # Calculate actual tickets to buy based on current dynamic state but limited by current Cap
            tickets_to_buy = min(current_n_tickets_base, cap)
            
            # Note: current_n_tickets_base tracks the "momentum" (2,3,4... up to 15 potentially)
            # But the actual purchase is clipped by the tier.
            # Example: current_n_tickets_base is 12 (high momentum).
            # If JP drops to 9亿 (someone failed but carryover still high?), Wait, usually carries over.
            # If JP drops below 10亿 but > 8亿... wait, JP only drops if won.
            # If not won, JP increases. So once above 10亿 it stays above til won.
            # So effectively once we cross 10亿, we unlock the higher cap.
            
            # --- PLAY ---
            sim_draws_played += 1
            w1 = counts1.astype(float) + ALPHA
            w2 = counts2.astype(float) + ALPHA
            penalty = PENALTY_BASE if tickets_to_buy < 5 else PENALTY_LARGE
            
            sim_cost += tickets_to_buy * TICKET_PRICE
            ...
            if jp_per_winner[i] > 0:
                ...
                # We allow base to grow up to 15, but actual usage is capped by tier logic above
                current_n_tickets_base = min(current_n_tickets_base + 1, MAX_TICKETS)
...
if __name__ == "__main__":
    ...
    print("\n" + "="*50)
    print(f"--- 分級策略 (8億Cap-10 -> 10億Cap-15) 分析報告 ---")
    print("="*50)
